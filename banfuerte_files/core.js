 ...
 * @version 1.1+
 * 
 * Define los objetos de interacción con el mapa
 *
 */
/* instancia global única de BMNWidgetMap */ 
var bmnLocatorMapWidgetInstance = null; 
/* instancia global de BMNLocationDetector, maneja la lógica de detección de ubicación */
...
// retorno después de carga de google maps
function locatorDOMReady() {
	...
	if( !BMNWidgetMapLocatorParams.fullLoaded ) {
		...
		bmnLocatorDetectorInstance = new BMNLocationDetector(function(status,geolocation,context) {
			bmnLocatorMapWidgetInstance.geoDetectorObj = context; // instancia de deteccción de ubicación
...
así como el control de filtros también para el localizador tradicional (clásico).
**/
function BMNWidgetMap(params) {
	/*
	Lógica para asignación de banderas tomando como referencia enteros positivos de 31 bits.
	...
	*/
	this.MKR_REG_SIZE = 25; /* tamaño inicial del registro de datos */
	...
	this.M_ICONS = [
		...
		"corresponsalia", // [10] corresponsalía lista
		"corresponsalia", // [11] corresponsalía mapa
	...
	this.currentLatLng = null; /* posición actual en el mapa */
	this.previousLatLng = null; /* posición de respaldo para centros pyme */
	this.map = null; /* MAPA de google */
	this.centerMarker = null; /* marca para el punto central de la búsqueda radial "MONITO" */
	...
	this.casasypymes = false; /* bandera para detectar si está en vista república o normal */
	this.watchID  = null; /* geo loc watch */
	this.enableGeoWatchLocation = false; /* habilita el modo watchLocation */
	this.currArea = null; /* círculo de área de búsqueda */
	...
	this.POS_ISJOINED = 23; /* posición de índice de punto fusionado (permite marcar los ignorados) */
	...
	this.MERGE_POINTS = true; /* Habilita la fusión de puntos muy cercanos */
	this.TEST_MODE = false; /* Muestra los puntos caché L1 */
	this.cachePositions = []; /* circulos auxiliares para delimitación de caché */
	this.BRAND_BANORTE = 8;
	this.BRAND_IXE = 1;
	this.BRAND_GFINANCIERO = 6;
	this.CR_LOCALIDAD = true; // true es CR, false es LOCALIDAD, modo en búsqueda tradicional
	...
	this.typeBankCenter = ""; // en búsqueda tradicional indica el tipo de centro para comboboxes
	this.queryDate = null; // fecha y hora en la que se realiza una búsqueda
	this.ENABLE_DRAG_AUTOUPDATE = false; // permite actualizar automáticamente la consulta de centros en el evento drag del mapa.
...
BMNWidgetMap.prototype = {
	...
	/* Lógica para click en los iconos de los centros */
...
//					$("#selectWaitingTimeTest").clone().appendTo(".selectWaitingTimeContainer");
	...
	createHTMLUSDMant: function(val) {
		var composite = '<ul class="phones dispDolares">Disposición dólares: '+((this.data[val][8])?'Si':'No')+'</ul>';
		if( this.data[val][9] ) {
			composite += '<ul class="phones"><strong style="color: #eb0029;">Cajero en mantenimiento.</strong></ul>';
		}
		return composite;
	},
	// Para las corresponsalías toma el campo 15 de tipo como parte del nombre,
	...
	createHTMLInfoBox: function(m,i,l, queueRequestEnable) {
		...
		for( key in mergedIdx ) {
			//console.log(key);
			if( key != i ) {
				composite2 += '<div class="title">';
				composite2 += '<div class="mapicon '+this.getIcon( mergedIdx[key],this.data[key][14],false,this.data[key][9] )+'" width="59" height="47"></div>';
				composite2 += '<span class="h1ReplaceCaj">'+this.data[key][1]+'</span><!--span class="cajMultiple"> Cajero múltiple: ' + this.data[key][25] + '</span--></div>'; // títulos
...
//		console.log(m.toString());
		...
		var t = '<div class="resultado_selected" id="globo-'+i+'"><div class="boxed">'+
			...
			this.getAbiertoCerrado(m,false)+'</div><div class="getthere"><a href="#" onclick="bmnLocatorMapWidgetInstance.routeToPoint('+i+');"><div src="" title="¿Cómo llegar?" alt="¿Cómo llegar?"></div>¿Cómo llegar?</a></div><div class="selectWaitingTimeContainer">';
...
//			}
			...
			if(m[15] == "SUC" && testWsQueuetimeEnable){
				t = t +	'<span>Seleccionar el tipo de trámite a realizar en esta sucursal para obtener el tiempo de espera: </span><br />'+
	...
	// Recorta el campo de dirección para que quede en una línea
	...
	getAbiertoCerrado: function(m, isLista) {
		...
		if( (m[15] == 'CJR' || m[15] == 'FUJ') && m[9] ) { // si es cajero y está en mantenimiento
	...
	initializeCommonWdgMapEvents: function() {
		...
		$('.widget_map input[type=checkbox]').click(function(e) {
			...
			if( !BMNWidgetMapLocatorParams.noGoogleMaps ) { // búsqueda mapa.
				obj.getDataAtCurrPos(false);
			}
			else { // búsqueda tradicional
...
//			console.log('headerGlobalOpen:' + headerGlobalOpen);
	...
	initializeLCoreWdgMapEvents: function() {
		...
		this.geocoder = new google.maps.Geocoder(); // dirección a la inversa, devuelve coordenadas
		...
		google.maps.event.addListener(this.map, 'dragend', function() {
			if( obj.ENABLE_DRAG_AUTOUPDATE ) {
				...
				obj.getDataAtCurrPos(false); // irá true TODO: Corregir globos incorrectamente creados
			}
		});
		...
		$('#maptxt_autocomplete').keyup(function(e) {
			...
			if(keycode == '13') {
				if( $(this).val() == '' ) {
	    			obj.displayMessage('Debe especificar una dirección.');
	    		}
			}
			else {
	...
	// Para validar la carga correcta del caché L1
	// Con TEST_MODE == true, muestra marcar de las posiciones (Caché L1) de donde se cargan los puntos de caché L2.
	showPositionsCache: function() {
		var obj = this;
		var timeNow = new Date().getTime();
		this.doAjaxS(BMNWidgetMapLocatorParams.gvUrl2, {}, function(resp) {
			// limpia posiciones de caché previas
	...
	onMapDblClick: function(event) {
		...
		// pinta la dirección en el campo de dirección de la ubicación donde se dió doble click
		this.reverseGeocoding();
		// actualiza la lista de puntos
		this.getDataAtCurrPos(false);
	},
	// Obtiene la dirección a partir de la lat y lng actual y la coloca en el campo de dirección
	reverseGeocoding: function() {
		...
		this.geocoder.geocode({'latLng': this.currentLatLng}, function(results, status) {
			var dir = 'Sin dirección en esta ubicación.';
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[1]) {
					dir = results[1].formatted_address;
				}
			} else {
				obj.displayMessage('Error al codificar la dirección ' + status);
			}
	...
	// Fusiona punto cajero y sucursal, sucursal tiene prioridad, se asume que el valor i es el índice
	// de la sucursal, el resto son cajeros, véase el algoritmo de "flagNearestPoints"
	...
	loadPoint: function(i, limit) {
		...
		if( this.data[i][this.POS_ISJOINED] && this.data[i][this.POS_JOINED] == null ) {
			// omite los marcados para fusión para ser ignorados
			isDrawable = false;
		}
...
//				console.log(i);
		 ...
		 else { // puntos omitidos, IMPORTANTE, permite manejar el mismo índice que los datos originales
	...
	// Auxiliar en la ubicación vertical del globo a 1/3 del alto del mapa respecto a 
	...
	// Lista de resultados para búsqueda tradicional sin Google Maps.
	loadList: function() {
	...
	loadPoints: function() {
		...
		if( this.data.length > 0 ) {
			// si la lista no está vacía y aplica el autozoom ajusta el mapa para visualizar todos los puntos
	...
	fillInAddress: function(hideMessage) {
		...
		if( place !== undefined && place.geometry !== undefined ) {
			...
			if( val != '' && val != $('#maptxt_autocomplete').attr('placeholder') ) {
				this.displayMessage('La dirección no es válida, debe seleccionarse de la lista desplegable, intente de nuevo.');
			}
			else if(!hideMessage) {
				this.displayMessage('Debe especificar una dirección.');
			}
		}
	},
	// Carga el codificador de direcciones en el campo de autocompletar dirección
	loadGeoCoder: function() {
		...
		if( htmlEl != null ) {
			this.autocomplete = new google.maps.places.Autocomplete(
		  	...
		  	// carga los datos de la dirección seleccionada de la lista
		  	var obj = this;
			google.maps.event.addListener(this.autocomplete, 'place_changed', function() { obj.fillInAddress(false); });
			obj.fillInAddress(true);
			// este listener es necesario para evitar la redirección en IE9 a la pag de indicadores
	...
	// Invoca la consulta de datos, dada una posición lat, lng
	...
	// Alimina el circulo de cobertura e ícono previos
	clearArea: function() {
	...
	// Crea o actualiza el círculo de cobertura radial de la búsqueda así como
	// el ícono central de ubicación detectada.
	drawArea: function(position) {
		...
		this.centerMarker = new google.maps.Marker({
			...
			title: "Mi ubicación detectada",
			clickable: true,
			map: this.map,
			icon: BMNWidgetMapLocatorParams.baseUrl+'images/monito.png'
		});
	},
	// Cierra la lista de resultados si está abierta
	...
	// expande lista de resultados si está colapsada
	...
	// Expande filtros secundarios solo si no están previamente abiertos
	...
	// Valida si el campo de dirección tiene contenido
	hasAddress: function() {
	...
	getDataAtCurrPos: function(stealthLoading) {
		...
		if( (params.flag & ( this.F_TIPO_CENTROPYME | this.F_TIPO_CASADEBOLSA )) !== 0  ) {
			...
			if( this.map ) {
				...
				// ajusta el zoom mínimo para centros cercanos
		...
		if( isLoad ) {
			...
			this.doAjaxS(BMNWidgetMapLocatorParams.gvUrl1, params, function(resp) {
				...
				if( obj.data.length > 0 && !obj.casasypymes ) {
					...
					obj.colapsaLista();
					obj.displayMessage('No se encontraron centros en la ubicación indicada.');
				}
				else {
					obj.hideResultadosList(true);
					obj.colapsaLista();
	...
	// El resultado de búsqueda llega como una cadenacon valores delimitados por == (filas) y | (columnas),
	// se genera un arreglo de 25 columnas y algunos datos cambian a tipo boolean o numérico para
	...
	parsePointsResponse: function(resp, obj) {
		...
		for( var key in rows ) {
			...
			if( cols.length >= obj.MKR_REG_SIZE-1 ) {
				...
				else { // nosmalmente no aplica solo si la geolocalización no está soportada
					cols[22] = 0;
				}*/
	...
	getInTime: function(ctx, dateString) {
		...
		if( mmin == 0 && mmax == 0 )
			isOnTime = true; // si los horarios son 0 indica que es todo el día
	...
	// Despliega un mensaje de estado en el panel deslizable debajo del campo de dirección
	...
	// Activa programáticamente un checkbox y actualiza la bandera de filtro
	on: function(sid) {
	...
	// Desactiva programáticamente un checkbox y actualiza la bandera de filtro
	off: function(sid) {
	...
	enableLinkedOptions: function(val, prevval) {
		...
		if( prevval == 0 ) { // ajuste por ningún checkbox forzado de al menos uno
		...
		if( this.isChk('#mapchk_sucursales') || this.isChk('#mapchk_cajeros') ) {
			// obtiene los filtros anteriores antes de la selección para intercambiar las casillas en grupo financiero
			...
			if( this.brand == this.BRAND_BANORTE) { // obliga a que la marca seleccionada en modo Banorte esté activa
				this.on('#mapchk_banorte');
			}
			else if( this.brand == this.BRAND_IXE) { // obliga a que la marca seleccionada en modo Ixe esté activa
				this.on('#mapchk_ixe');
			}
			else { // las demás marcas Grupo Financiero
				if( !prevBanorte && !prevIxe ) { // si inicialmente no se tenía ningún valor activa Banorte e Ixe
					this.on('#mapchk_banorte');
					this.on('#mapchk_ixe');
				}
				// si la nueva selección deja Banorte e Ixe desactivados
			...
			if( !this.isChk('#mapchk_cajeros') ) { // si solo están activas las sucursales desactiva filtros de cajeros
			...
			//if( this.mapCompatible ) {
				this.colapsaFiltros();
			//}
			// oculta la opción para los subfiltros
		...
		if(!this.isChk('#mapchk_corresponsalias'))
		{
			...
			this.offsub( ["#mapchk_oxxo","#mapchk_7eleven","#mapchk_circulok","#mapchk_delsol","#mapchk_farmaciagdl","#mapchk_netpaygasmart","#mapchk_soriana","#mapchk_telecom","#mapchk_tiendas-extra","#mapchk_woolworth"] );
			// oculta la opción para los subfiltros
	...
	// Habilita / Dehabilita programáticamente y visualmente un checkbox indicando su id
	...
	// Lo mismo que la función off pero aplicada a una lista de id's de checkboxes
	offsub: function(checkList) {
		for( var h=0; h<checkList.length; ++h) {
			this.off(checkList[h]);
		}
	},
	// Muestra el indicador de carga bloqueando los demás controles del widget y evitar duplicidad de invocaciones
	...
	// Permite hacer invocaciones vía ajax con detección de errores.
	doAjaxS: function(url, params, successCallback) {
		...
		$.ajax({
			...
			success: function(resp) {
				...
				//obj.setLoadingMask(false); // se comentó por delegar esta acción al terminar de procesar los datos
				successCallback(resp);
			},
			error: function(xhr, textStatus, thrownError){
				if( this.TEST_MODE ) { console.log('error ' + xhr.status + ': ' + thrownError); }
				obj.displayMessage('Por el momento no está disponible la información, intenta más tarde.');
				obj.setLoadingMask(false);
			}
		});
	},
	...
	// Corrige el tamaño del mapa si fue correctamente inicializado
	...
	// Activa el botón de detección de ubicación e invoca la actualización de la posición 
	...
	initMaster: function() {
		...
		else { // Permite la adición dinámica del widget al home de portal
	...
	// Crea un elemento para la lista de resultados búsqueda tradicional
	...
	// Carga via ajax el contenido del combobox en búsqueda tradicional
	...
	initializeClassicWdgMapEvents: function(ctx) {
		$('#mapcb_tipo_centro').change(function(e) {
			...
			if( val == ctx.F_SUCURSAL ) {
				...
				ctx.resetCRLocalidadView(ctx);
				ctx.typeBankCenter = ""; // TODO: obtener solo los filtros de corresponsalías en backend
				/*$('#map_classic_results').addClass('buscarcr-wrapper');
				$('#map_classic_results .filtros').hide();*/
				$('#mapbtn_buscarclc').attr('disabled', true);
			}
	...
	// Traza la ruta como llegar del punto central de la búsqueda, al centro banorte seleccionado
	...
	// Fusión de puntos: Crea o actualiza un "hashmap" con los índices v1 y v2 
	...
	// Fusión de puntos: Valida si cualquiera de los dos índices v1 y v2 se encuentran en un hashmap existente
	...
	*/
	...
	// Fusión de puntos: Marca los puntos para fusionar tomando como criterio que esten localizados
	// entre si a una distancia igual o menor de this.radiuz
	flagNearestPoints: function(obj) {
		var len = obj.data.length;
		var ini = 0, fin = len;
		obj.fusee = []; // limpia la lista previa
		// obtiene los índices de los puntos a fusionar, los coloca en una lista de grupos denominada fusee,
...
/*
Aplica cuando termina la animación de la página de inicio, proviene de header.js
...
function mapWidgetPostInitialize() {
    ...
    getScript(BMNWidgetMapLocatorParams.googleapisUrl,
    	function() { // Success el retorno será por locatorDOMReady
        	console.log('API de Mapas OK.');
        	BMNWidgetMapLocatorParams.noGoogleMaps = false;
    	},
    	function() { // Error continúa a locatorDOMReady de forma explícita
...
/**
 * Maneja la lógica de detección de ubicación 
*/
BMNLocationDetector.prototype = {
	...
	failed: function(obj) { 
	    ...
	    else { // no selecciona opción ni permitida
	...
	queryLocationStatus: function() {
		...
		if ( navigator.geolocation ) { 
		    ...
		    //console.log('Localización no soportado.');
	...
	getLocation: function() {
		...
		if( this.enableGeoWatchLocation ) {
			...
			this.watchID = navigator.geolocation.watchPosition( 
				...
				function(error) {
					mapW.displayMessage('La detecci�n de ubicaci�n no fue autorizada previamente. Verifique la configuraci�n del Navegador Web.');
...
/*
lógica de detección de ubicación END
*/
...
if(window.location.href.indexOf('www.banorte.com')>0){
	...
	//COMPONENTE UBICADO EN Bibliotecas/Contenido Web Sitio Publico Banorte /Componentes/Componentes de página de inicio/wsBranchSettings.js
